...
esp_err_t playback_file(const char *path, bool duck_others) {
  ...
  ESP_GOTO_ON_ERROR(audio_pipeline_register(pipeline, file_stream, "file"),
                    cleanup, RADIO_TAG,
                    "Failed to register file stream to pipeline");
  ...
  if (ext != NULL && strcasecmp(ext, ".opus") == 0) {
    ...
    ESP_GOTO_ON_FALSE(decoder, ESP_FAIL, cleanup, RADIO_TAG,
  ...
  ESP_GOTO_ON_ERROR(audio_pipeline_register(pipeline, decoder, "decoder"),
                    cleanup, RADIO_TAG,
                    "Failed to register decoder to pipeline");
  ESP_GOTO_ON_ERROR(audio_element_set_output_ringbuf(decoder, output), cleanup,
                    RADIO_TAG, "Failed to set output ringbuf for decoder");

  ESP_GOTO_ON_ERROR(
      audio_pipeline_link(pipeline, (const char *[]){"file", "decoder"}, 2),
      cleanup, RADIO_TAG, "Failed to link file and decoder to pipeline");

  audio_event_iface_cfg_t evt_cfg = AUDIO_EVENT_IFACE_DEFAULT_CFG();
  evt = audio_event_iface_init(&evt_cfg);
  ESP_GOTO_ON_FALSE(evt, ESP_FAIL, cleanup, RADIO_TAG,
                    "Failed to create event interface");
  ESP_GOTO_ON_ERROR(audio_pipeline_set_listener(pipeline, evt), cleanup,
                    RADIO_TAG, "Failed to set listener for pipeline");
  ...
  while (1) {
    ...
    if (msg.cmd == AEL_MSG_CMD_REPORT_MUSIC_INFO) {
      ...
      ESP_GOTO_ON_ERROR(audio_element_getinfo(decoder, &info), cleanup,
                        RADIO_TAG, "Failed to get decoder info");
      ESP_LOGD(RADIO_TAG, "Playback audio format (%s): %d Hz, %d bits, %s",
               path, info.sample_rates, info.bits,
               info.channels == 1 ? "mono" : "stereo");

      if (!channel) {
        ESP_GOTO_ON_ERROR(mixer_play_audio((mixer_read_callback_t)rb_read,
                                           output, info.sample_rates, info.bits,
                                           info.channels, duck_others,
                                           &channel),
                          cleanup, RADIO_TAG, "Failed to add audio to mixer");
      }
