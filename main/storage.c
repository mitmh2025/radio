...
#define BLOCK_FLASH_BLOCK_COUNT                                                \
  (16 /* erase sectors per block */ * 256 /* blocks per chip */)
#define BLOCK_FLASH_SIZE_BYTES                                                 \
  (BLOCK_FLASH_ERASE_SIZE * BLOCK_FLASH_BLOCK_COUNT)
...
static void storage_telemetry_generator() {
  if (block_type == BLOCK_FLASH) {
    ...
    int64_t total_write_ops = 0;
    int64_t total_write_bytes = 0;
    int64_t total_erase_ops = 0;

    for (int i = 0; i < BLOCK_FLASH_COUNT; i++) {
      xSemaphoreTake(block_flash[i].mutex, portMAX_DELAY);
      total_read_ops += block_flash[i].stats.read_ops;
      total_read_bytes += block_flash[i].stats.read_bytes;
      total_write_ops += block_flash[i].stats.write_ops;
      total_write_bytes += block_flash[i].stats.write_bytes;
      total_erase_ops += block_flash[i].stats.erase_ops;
      xSemaphoreGive(block_flash[i].mutex);
    }

    things_send_telemetry_int("storage_read_ops", total_read_ops);
    things_send_telemetry_int("storage_read_bytes", total_read_bytes);
    things_send_telemetry_int("storage_write_ops", total_write_ops);
    things_send_telemetry_int("storage_write_bytes", total_write_bytes);
...
static esp_err_t flash_init(block_flash_t *flash, spi_device_handle_t device) {
  ...
  ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
  ...
  ESP_RETURN_ON_ERROR(flash_wait_ready(flash, portMAX_DELAY), RADIO_TAG,
                      "SPI flash did not become ready");
  ...
  txn.rx_buffer = NULL;
  ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
                      "Failed to enable reset on SPI flash");
  txn.cmd = BLOCK_FLASH_CMD_RESET;
  ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
                      "Failed to reset SPI flash");

  // Wait for reset to complete
  ets_delay_us(50);

  flash->ready = false;
  ESP_RETURN_ON_ERROR(flash_wait_ready(flash, pdMS_TO_TICKS(1)), RADIO_TAG,
                      "SPI flash did not become ready after reset");
  ...
  txn.rxlength = 8;
  ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
                      "Failed to read SPI flash status register 2");
  ...
  if (!reg2.parsed.quad_enable) {
    ...
    txn.rx_buffer = NULL;
    ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
                        "Failed to enable register write on SPI flash");
    ...
    txn.rx_buffer = NULL;
    ESP_RETURN_ON_ERROR(spi_device_transmit(device, &txn), RADIO_TAG,
                        "Failed to enable quad mode on SPI flash");

    ESP_RETURN_ON_ERROR(
        flash_wait_ready(flash, pdMS_TO_TICKS(1)), RADIO_TAG,
        "SPI flash did not become ready after enabling quad mode");
  }
...
static esp_err_t flash_read(block_flash_t *flash, size_t addr, void *buffer,
                            size_t size) {
  ...
  ESP_GOTO_ON_ERROR(flash_wait_ready(flash, portMAX_DELAY), cleanup, RADIO_TAG,
                    "SPI flash did not become ready");
  ...
  ESP_GOTO_ON_ERROR(
      spi_device_transmit(flash->handle, &txn.base), cleanup, RADIO_TAG,
      "Failed to read from SPI flash: addr 0x%08zx, size %zu", addr, size);
  flash->stats.read_ops++;
  flash->stats.read_bytes += size;
...
static esp_err_t flash_write(block_flash_t *flash, size_t addr,
  ...
  ESP_GOTO_ON_ERROR(flash_wait_ready(flash, portMAX_DELAY), cleanup, RADIO_TAG,
                    "SPI flash did not become ready");
  ESP_GOTO_ON_ERROR(flash_enable_write(flash), cleanup, RADIO_TAG,
                    "Failed to enable write on SPI flash");
  ...
  ESP_GOTO_ON_ERROR(
      spi_device_transmit(flash->handle, &txn.base), cleanup, RADIO_TAG,
      "Failed to write to SPI flash: addr 0x%08zx, size %zu", addr, size);
  ...
  flash->stats.write_ops++;
  flash->stats.write_bytes += size;
...
static esp_err_t flash_erase(block_flash_t *flash, size_t addr) {
  ...
  ESP_GOTO_ON_ERROR(spi_device_transmit(flash->handle, &txn.base), cleanup,
                    RADIO_TAG, "Failed to erase SPI flash: addr 0x%08zx", addr);

  int64_t start = esp_timer_get_time();
  // Typical sector erase time is 45ms with max of 400ms, but allow up to 1
  // second
  vTaskDelay(pdMS_TO_TICKS(45));
  ESP_GOTO_ON_ERROR(flash_wait_ready(flash, pdMS_TO_TICKS(1000)), cleanup,
                    RADIO_TAG, "SPI flash did not become ready after erase");
  int64_t end = esp_timer_get_time();
  if (end - start > 401000) {
    ESP_LOGW(RADIO_TAG, "SPI flash erase took %lldus", end - start);
  }

  flash->stats.erase_ops++;
...
int block_read(void *context, uint32_t block, uint32_t offset, void *buffer,
  ...
  if (end_addr < start_addr ||
      end_addr > BLOCK_FLASH_SIZE_BYTES * BLOCK_FLASH_COUNT) {
    return -EINVAL;
  }
  ...
  for (int idx = first_chip; idx <= last_chip; idx++) {
    ...
    size_t flash_read_start =
        start_addr > flash_start ? start_addr : flash_start;
    ...
    for (int sector = first_sector; sector <= last_sector; sector++) {
      ...
      size_t read_start =
          flash_read_start > sector_start ? flash_read_start : sector_start;
      size_t read_end =
          flash_read_end < sector_end ? flash_read_end : sector_end;
      size_t read_size = read_end - read_start;

      esp_err_t ret = flash_read(flash, read_start % BLOCK_FLASH_SIZE_BYTES,
                                 buffer, read_size);
      if (ret != ESP_OK) {
        ESP_LOGE(
            RADIO_TAG,
...
static int block_write(void *context, uint32_t block, uint32_t offset,
                       const void *buffer, size_t size) {
  ...
  for (int idx = first_chip; idx <= last_chip; idx++) {
    ...
    for (int sector = first_sector; sector <= last_sector; sector++) {
      ...
      size_t write_start =
          flash_write_start > sector_start ? flash_write_start : sector_start;
      size_t write_end =
          flash_write_end < sector_end ? flash_write_end : sector_end;
      size_t write_size = write_end - write_start;

      esp_err_t ret = flash_write(flash, write_start % BLOCK_FLASH_SIZE_BYTES,
                                  buffer, write_size);
      if (ret != ESP_OK) {
        ESP_LOGE(
            RADIO_TAG,
