...
static esp_err_t adc_config_and_start() {
  ...
  xTimerStart(adc_watchdog, portMAX_DELAY);
...
static void adc_task(void *context) {
  ...
  ESP_ERROR_CHECK_WITHOUT_ABORT(adc_config_and_start());
  xSemaphoreGive(adc_mutex);

  while (true) {
    uint32_t notification;
    xTaskNotifyWait(0, ULONG_MAX, &notification, pdMS_TO_TICKS(1000));
    if (notification & NOTIFY_ADC_RESTART) {
      xSemaphoreTake(adc_mutex, portMAX_DELAY);
      if (adc_running) {
        adc_continuous_stop(adc_handle);
        adc_running = false;
        xTimerStop(adc_watchdog, portMAX_DELAY);
      }
      ESP_ERROR_CHECK_WITHOUT_ABORT(adc_config_and_start());
      xSemaphoreGive(adc_mutex);
    }
...
esp_err_t adc_init() {
  ...
  adc_watchdog = xTimerCreate("adc_watchdog", pdMS_TO_TICKS(1000), pdTRUE, 0,
                              adc_watchdog_cb);
  ESP_RETURN_ON_FALSE(adc_watchdog, ESP_ERR_NO_MEM, RADIO_TAG,
                      "Failed to create ADC watchdog");
