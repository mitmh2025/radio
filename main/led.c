...
esp_err_t led_set_pixel(uint32_t index, uint32_t red, uint32_t green,
                        uint32_t blue) {
  ESP_RETURN_ON_FALSE(led_mutex, ESP_ERR_INVALID_STATE, RADIO_TAG,
                      "led_init must be called before led_set_pixel");
  ESP_RETURN_ON_FALSE(index < LED_COUNT, ESP_ERR_INVALID_ARG, RADIO_TAG,
                      "index must be less than LED_COUNT");
  xSemaphoreTake(led_mutex, portMAX_DELAY);
  ...
  ESP_GOTO_ON_ERROR(led_strip_refresh(led), cleanup, RADIO_TAG,
                    "led_set_pixel flush failed");
