...
esp_err_t debounce_handler_add(gpio_num_t gpio_num, gpio_int_type_t intr_type, gpio_isr_t isr_handler, void *args, uint64_t timeout_us)
{
  ESP_RETURN_ON_FALSE(GPIO_IS_VALID_GPIO(gpio_num), ESP_ERR_INVALID_ARG, RADIO_TAG, "Invalid GPIO number: %d", gpio_num);
  ESP_RETURN_ON_FALSE(intr_type == GPIO_INTR_POSEDGE || intr_type == GPIO_INTR_NEGEDGE || intr_type == GPIO_INTR_ANYEDGE, ESP_ERR_INVALID_ARG, RADIO_TAG, "Invalid interrupt type: %d", intr_type);
  ...
  ESP_RETURN_ON_ERROR(gpio_set_intr_type(gpio_num, GPIO_INTR_ANYEDGE), RADIO_TAG, "Failed to set interrupt type");
  ESP_RETURN_ON_ERROR(gpio_intr_enable(gpio_num), RADIO_TAG, "Failed to enable interrupt");
...
{
  ESP_RETURN_ON_FALSE(GPIO_IS_VALID_GPIO(gpio_num), ESP_ERR_INVALID_ARG, RADIO_TAG, "Invalid GPIO number: %d", gpio_num);
  ESP_RETURN_ON_ERROR(gpio_isr_handler_remove(gpio_num), RADIO_TAG, "Failed to remove ISR handler");
  portENTER_CRITICAL(&debounce_spinlock);
