...
static void wifi_report_telemetry() {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(RADIO_TAG, "Failed to get AP info: %d (%s)", err,
             esp_err_to_name(err));
    return;
  }
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT) {
    ...
    case IP_EVENT_STA_GOT_IP: {
      ...
      if (err != ESP_OK) {
        ESP_LOGE(RADIO_TAG, "Failed to start SNTP: %d (%s)", err,
                 esp_err_to_name(err));
      }
...
esp_err_t wifi_init() {
  ...
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to initialize wifi: %s",
                      esp_err_to_name(err));

  // TODO: reevaluate if this is what we actually want (or if, e.g., we only
  // want it while streaming audio)
  err = esp_wifi_set_ps(WIFI_PS_NONE);
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to set wifi power save mode: %s",
                      esp_err_to_name(err));

  err = esp_netif_init();
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to initialize netif: %s",
                      esp_err_to_name(err));
  ...
  ESP_RETURN_ON_ERROR(err, RADIO_TAG,
                      "Failed to register wifi event handler: %s",
                      esp_err_to_name(err));
  err = esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID,
                                            &wifi_event_handler, NULL, NULL);
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to register ip event handler: %s",
                      esp_err_to_name(err));
  ...
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to set wifi mode: %s",
                      esp_err_to_name(err));
  err = esp_wifi_set_config(WIFI_IF_STA, &wifi_config);
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to set wifi config: %s",
                      esp_err_to_name(err));
  err = esp_wifi_start();
  ESP_RETURN_ON_ERROR(err, RADIO_TAG, "Failed to start wifi: %s",
                      esp_err_to_name(err));
