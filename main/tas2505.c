...
static const tas2505_cfg_reg_t tas2505_init_registers[] = {
    ...
    {TAS2505_CFG_OP_SET_REG, TAS2505_CFG_REG_SPEAKER_VOLUME_RANGE, 0x20},
...
static esp_err_t tas2505_write_register(uint8_t offset, uint8_t value) {
  ...
  esp_err_t ret =
      i2c_master_transmit(i2c_device, data, sizeof(data) / sizeof(data[0]), -1);
  return ret;
}

static esp_err_t tas2505_read_register(uint8_t offset, uint8_t *value) {
  esp_err_t ret =
      i2c_master_transmit_receive(i2c_device, &offset, 1, value, 1, -1);
  return ret;
}

static esp_err_t tas2505_write_registers(const tas2505_cfg_reg_t *registers,
                                         size_t len) {
    ...
    default: {
      ...
      if (current_page != page) {
        ...
        ESP_GOTO_ON_ERROR(ret, end, TAG,
                          "i2c_bus_write_bytes: page=%d (idx=%d)", page, i);
        current_page = page;
      }

      uint8_t val = registers[i].value;
      if (registers[i].op == TAS2505_CFG_OP_SET_BITS ||
          registers[i].op == TAS2505_CFG_OP_CLEAR_BITS) {
        ret = tas2505_read_register(offset, &val);
        ESP_GOTO_ON_ERROR(ret, end, TAG, "i2c_bus_read_bytes: reg=%d (idx=%d)",
                          offset, i);
...
static esp_err_t _force_tas2505_set_output(tas2505_output_t output) {
  ...
  case TAS2505_OUTPUT_SPEAKER:
    ...
    ret = tas2505_write_registers(
        tas2505_speaker_output_registers,
        sizeof(tas2505_speaker_output_registers) /
            sizeof(tas2505_speaker_output_registers[0]));
    break;
  case TAS2505_OUTPUT_HEADPHONE:
    ESP_LOGI(TAG, "Setting output to headphone");
    ret = tas2505_write_registers(
        tas2505_headphone_output_registers,
        sizeof(tas2505_headphone_output_registers) /
            sizeof(tas2505_headphone_output_registers[0]));
    break;
  case TAS2505_OUTPUT_BOTH:
    ESP_LOGI(TAG, "Setting output to both");
    ret = tas2505_write_registers(tas2505_both_output_registers,
                                  sizeof(tas2505_both_output_registers) /
                                      sizeof(tas2505_both_output_registers[0]));
    break;
  default:
...
static esp_err_t _force_tas2505_set_input(tas2505_input_t input) {
  ...
  case TAS2505_INPUT_DAC:
    ...
    ret = tas2505_write_registers(tas2505_line_off_registers,
                                  sizeof(tas2505_line_off_registers) /
                                      sizeof(tas2505_line_off_registers[0]));
    ESP_RETURN_ON_ERROR(ret, TAG, "Setting input to DAC failed");
    ret = tas2505_write_registers(tas2505_dac_on_registers,
                                  sizeof(tas2505_dac_on_registers) /
                                      sizeof(tas2505_dac_on_registers[0]));
    break;
  case TAS2505_INPUT_LINE:
    ESP_LOGI(TAG, "Setting input to line");
    ret = tas2505_write_registers(tas2505_dac_off_registers,
                                  sizeof(tas2505_dac_off_registers) /
                                      sizeof(tas2505_dac_off_registers[0]));
    ESP_RETURN_ON_ERROR(ret, TAG, "Setting input to line failed");
    ret = tas2505_write_registers(tas2505_line_on_registers,
                                  sizeof(tas2505_line_on_registers) /
                                      sizeof(tas2505_line_on_registers[0]));
    break;
  case TAS2505_INPUT_BOTH:
...
esp_err_t tas2505_enable_pa(bool enable) {
  ...
  if (enable) {
    ...
    esp_err_t ret = tas2505_write_registers(
        tas2505_init_registers,
        sizeof(tas2505_init_registers) / sizeof(tas2505_init_registers[0]));
...
esp_err_t tas2505_read_gpio(bool *val) {
  ...
  BOARD_I2C_MUTEX_LOCK();
  ret = tas2505_write_register(0x0, page);
  ESP_GOTO_ON_ERROR(ret, end, TAG, "i2c_bus_write_bytes: switch to page=%d",
                    page);
...
esp_err_t tas2505_set_volume(uint8_t volume) {
  ...
  for (size_t i = 0; i < sizeof(volume_registers) / sizeof(volume_registers[0]);
       i++) {
    volume_registers[i].value = value;
  }

  esp_err_t err = tas2505_write_registers(
      volume_registers, sizeof(volume_registers) / sizeof(volume_registers[0]));
